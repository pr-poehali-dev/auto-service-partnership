...
const SERVICES = [
  ...
  },
  {
    ...
    tag: "Импорт",
    price: "Каталог →",
    desc: "Haval, Chery, Geely, BYD и другие. Под ключ: авто + доставка + таможня + все документы до г. Уссурийск.",
    badge: "Хит",
    badgeColor: "bg-orange text-white",
  ...
  },
  {
    ...
    tag: "Импорт",
    price: "Каталог →",
    desc: "Hyundai, Kia, Genesis и другие. Под ключ: авто + доставка + таможня + все документы до г. Уссурийск.",
    badge: "Популярно",
            ...
            {filtered.map((s) => (
              <div key={s.id} className={`card-lift bg-white border border-surface-4 rounded-2xl p-6 flex flex-col gap-4 relative ${"widgetId" in s && s.widgetId ? "md:col-span-2 lg:col-span-1" : ""}`}>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div style={{fontFamily: "Arial, sans-serif", fontSize: "13px", color: "#888", fontWeight: 400, paddingRight: "90px"}}>{s.tag}</div>
